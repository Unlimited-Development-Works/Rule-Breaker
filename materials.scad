...
module jitter_color(c) {
    amount = 0.1;
    d = [
        min(1, max(0, c[0] + c[0] * rands(-amount,amount,1)[0])),
        min(1, max(0, c[1] + c[1] * rands(-amount,amount,1)[0])),
        min(1, max(0, c[2] + c[2] * rands(-amount,amount,1)[0]))
    ];
    color(d) children();
}
...
module PLA() {
    printed("PLA") jitter_color([100 / 255, 149 / 255, 237 / 255]) children();
}

module PETG() {
    printed("PETG") jitter_color([57 / 255, 255 / 255, 11 / 255]) children();
}
